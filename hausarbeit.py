...
# Konfusionsmatrix für k-Means und SVM
fig, axes = plot.subplots(1, 2, figsize=(20, 8)) # Parameter Erklärung: Die Zahlen 1 und 2 bedeuten, dass eine Zeile und zwei Spalten erzeugt werden. Die Größe beträgt 20 Zoll Breite und 5 Zoll Höhe.
kf_kmeans = confusion_matrix(Y_test, pred_kmeans) # Vergleich mit wahren Labels (Y_test) und den Vorhersagen (pred_kmeans).
kf_svm = confusion_matrix(Y_test, pred_svm) # Vergleich mit wahren Labels (Y_test) und den Vorhersagen (pred_svm).
